...
const ProjectWorking = () => {
  ...
  const fetchProject = async (id) => {
    await axios
      .get(`http://localhost:6001/fetch-project/${id}`)
      .then((response) => {
  ...
  const handleApproveSubmission = async () => {
    await axios
      .get(`http://localhost:6001/approve-submission/${params["id"]}`)
      .then((response) => {
        fetchProject(params["id"]);
  ...
  const handleRejectSubmission = async () => {
    await axios
      .get(`http://localhost:6001/reject-submission/${params["id"]}`)
      .then((response) => {
        fetchProject(params["id"]);
  ...
  const fetchChats = async () => {
    await axios
      .get(`http://localhost:6001/fetch-chats/${params["id"]}`)
      .then((response) => {
                ...
                <div className="project-submissions">
                  {project.submission ? (
                    <div className="project-submission">
                      ...
                      <span>
                        <h5>Manual Link: </h5>
                        <a href={project.manulaLink} target="_blank">
                          {project.manulaLink}
                        </a>
                      </span>
                    ...
                    {chats.messages.map((message) => (
                      ...
                      >
                        <div>
                          <p>{message.text}</p>
                          <h6>
                            {message.time.slice(5, 10)} -{" "}
                            {message.time.slice(11, 19)}
                          </h6>
                        </div>
                      </div>
                    ))}
